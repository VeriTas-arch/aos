...
class beckhoff_device(object):
    ...
    def basicInterface(self):
        override = self.plc.read_by_name("MC_FIFO.stFifoInterface.rOverride", pyads.PLCTYPE_REAL)
        print("override=", override)

        override_flag = self.plc.read_by_name("MC_FIFO.stFifoInterface.bSetOverride", pyads.PLCTYPE_BOOL)
        ...
        if directWrite or (self.AdsCallEvent.wait(wait) and self.AdsCallEvent.is_set()):
            ...
            if dataLen > 0:
                self.plc.write_by_name(
                    "MC_FIFO.stADSSharedMem.arrOutput", memRet, pyads.PLCTYPE_LREAL * dataLen
                )
