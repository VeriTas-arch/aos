...
class plane_part_stewart_driver(object):
    def __init__(self, name):
        ...
        basicPlanner = MXController.trajectoryPlanner(
        ...
        self.ptp_sub = rospy.Subscriber(
            "/stewart_ptp_command", Float64MultiArray, self.ptp_cb
        )
    ...
    def ptp_cb(self, msg):
        ...
        this_f = msg.data
        mat_traj = transformations.euler_matrix(
            this_f[3], this_f[4], this_f[5], axes="rxyz"
        )
    ...
    def runCycle(self):
        ...
        while not rospy.is_shutdown():
            simuLegLength = self._device.readLegControlLength()
            trans_mat_traj = self.jointStateController.getJointState(
                simuLegLength, initGuess=ig
            )
            ig = trans_mat_traj
            ...
            for i in range(len(joint_states)):
                self._model.name_position_dict[self._model.joint_names[i]] = (
                    joint_states[i]
                )
    ...
    def execute_cb(self, goal):
        ...
        simuLegLength = self._device.readLegControlLength()
        tempPlanner = MXController.JointStateController(
            self.stewart, self.stewart_calibration
        )
        ...
        if delta > 6e-3:
            ...
            self.mxplanner = MXController.MXController(
                self.stewart, traj, self.stewart_calibration
            )
