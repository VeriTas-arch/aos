...
class JointStateController(trajectoryPlanner):
    ...
    def getJointState(self, startConf, initGuess = None):
        ...
        currPose, iterc, deltaLen = self.stewart.forwardKin(
            startConf + self.fixed_leg_length, initGuess, 
            eps=3e-6, maxiter=20)

        return currPose

class PtPController(trajectoryPlanner):
    def __init__(self,stewart, stewart_calibration, startConf, tgtConf, timeInterval = 5.0, mode='Pose2Pose'):
        ...
        if(mode=='Leg2Pose' or mode =='Leg2Leg'):
            currPose, iterc, deltaLen = self.stewart.forwardKin(
                startConf + self.fixed_leg_length, self.bottom2Platform,
                eps=3e-6, maxiter=20)
        else:
            currPose = startConf
        ...
        if(mode == 'Leg2Leg'):
            tgtPose, iterc, deltaLen = self.stewart.forwardKin(
                tgtConf + self.fixed_leg_length, self.bottom2Platform,
                eps=3e-6, maxiter=20)
        else:
            tgtPose = tgtConf
...
class MXController(trajectoryPlanner):
    def __init__(self,stewart,tgtPlatformTraj,stewart_calibration,bufsize=50,tick = 0.01):
        ...
        for i in range(sz[0]):
            ...
            thisq = quaternion.from_rotation_matrix(smb.rpy2r(thisEuler[2],thisEuler[1],thisEuler[0],order='xyz'))
            if(thisq.w<0):
                thisq = -thisq
...
if __name__ == '__main__':
    ...
    tgtConf = basicPlanner.bottom2Platform @ smb.rt2tr(smb.rpy2r(math.pi/180.0*0.0,0.0,0.0), np.array([0,0,0.0045]))
