...
class trajectoryPlanner(object):
    def __init__(self, stewart, stewart_calibration):
        ...
        self.bottom2Platform = matrixHelper.getRelativeTransformMatrix(
            stewart_calibration["bottom_transform"],
            stewart_calibration["platform_transform"],
        )
...
class PtPController(trajectoryPlanner):
    def __init__(
        ...
        if mode == "Leg2Pose" or mode == "Leg2Leg":
            currPose, iterc, deltaLen = self.stewart.forwardKin(
                startConf + self.fixed_leg_length,
                self.bottom2Platform,
                eps=3e-6,
                maxiter=20,
            )
        else:
    ...
    def trajectoryPlanningPtp(
        self, MatTargetInitPose, MatTargetPose, interval, tick=0.01
    ):
        # p2p planning algorithm from the initial position to the starting point
        timeTraj = rtb.tools.trajectory.trapezoidal(
            1.0e-10, 1.0 - 1e-10, np.arange(0, interval, tick)
        )
    ...
    def RunControlStep(self, col, row):
        if self.framePointer + row < self.maxLen:
            ...
            memRet = np.reshape(
                self.buffer[self.framePointer : self.maxLen, :], (1, -1)
            ).tolist()[0]
...
class MXController(trajectoryPlanner):
    def __init__(
        self, stewart, tgtPlatformTraj, stewart_calibration, bufsize=50, tick=0.01
    ):
    ...
    def CarculateMxTraj(self):
        ...
        for i in range(timeTicks.shape[0]):
            ...
            self.history = np.row_stack(
                [self.history, quaternion.as_rotation_vector(rst[i])]
            )
            ...
            self.buffer = np.row_stack(
                [self.buffer, thisLegLen - self.fixed_leg_length]
            )
...
if __name__ == "__main__":
    ...
    stewart_calibration = {}
    stewart_calibration["bottom_transform"] = np.array(
        rospy.get_param("/calibration/bottom_transform")
    )
    stewart_calibration["platform_transform"] = np.array(
        rospy.get_param("/calibration/platform_transform")
    )
    stewart_calibration["fixed_leg_length"] = np.array(
    ...
    ptpplanner = PtPController(
        stewart, stewart_calibration, simuLegLength, startConf, mode="Leg2Pose"
    )
