...
class agv_controller:
    def __init__(self):
        ...
        self.state_publisher = rospy.Publisher('/agv_state', Int8, queue_size=10)  # 0: not moving; 1: moving
    ...
    def adjust_axis(self, target_value, get_current_pos, direction_sign, speed, tolerance, max_attempts):
        ...
        for attempt in range(1, max_attempts+1):
            ...
            if abs(error) < tolerance:
                ...
                rospy.loginfo(f"{direction_sign.upper()} aligned (Error: {error:.3f}{self.get_unit(direction_sign)})")
                return True
            ...
            rospy.loginfo(f"Attempt {attempt}/{max_attempts}: Error={error:.3f}{self.get_unit(direction_sign)}")
